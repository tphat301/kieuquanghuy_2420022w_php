	...
	<div class="flex-cus row">
		<?php foreach ($items as $k => $v) { ?>
			<div class="col-6 col-sm-6 col-md-4 col-lg-4 mg-cus mb-3">
				<div class="box-product box-product-edit" data-aos="fade-up" data-aos-duration="1000">
					<div class="pic-product">
						<a class="text-decoration-none scale-img" href="<?= $v[$sluglang] ?>" title="<?= $v['name' . $lang] ?>">
							<img class="lazy w-100" onerror="this.src='<?= THUMBS ?>/380x300x1/assets/images/noimage.png';" data-src="<?= WATERMARK ?>/product/380x300x2/<?= UPLOAD_PRODUCT_L . $v['photo'] ?>" alt="<?= $v['name' . $lang] ?>" title="<?= $v['name' . $lang] ?>" />
						</a>
					</div>
					<div class="box-name-price-product">
