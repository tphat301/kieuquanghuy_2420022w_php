...
$static = $d->rawQueryOne("select id, type, name$lang, desc$lang, photo, photo1 from #_static where type = ? limit 0,1", array('gioi-thieu'));
...
$khong_gian_quan = $cache->get("select photo, slugvi, name$lang from #_product where type = ? and find_in_set('hienthi',status) order by numb asc", array('thu-vien-anh'), 'result', 7200);
$cam_nhan_khach_hang = $cache->get("select name$lang, slugvi, content$lang, office$lang, desc$lang, photo from #_news where type = ? and find_in_set('noibat',status) and find_in_set('hienthi',status) order by id desc", array('cam-nhan-khach-hang'), 'result', 7200);
